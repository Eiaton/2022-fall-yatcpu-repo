...
class InstructionDecode extends Module {
  ...
  io.ex_aluop2_source := MuxLookup(
    opcode,
    ALUOp2Source.Register  // 默认情况为来自寄存器
  )(
    Seq(
      InstructionTypes.I -> ALUOp2Source.Immediate,
      InstructionTypes.L -> ALUOp2Source.Immediate,
      InstructionTypes.S -> ALUOp2Source.Immediate,
      InstructionTypes.B -> ALUOp2Source.Immediate,
      Instructions.lui -> ALUOp2Source.Immediate, 
      Instructions.auipc -> ALUOp2Source.Immediate
    )
  )
  ...
  io.wb_reg_write_source := MuxLookup(
    ...
    Seq(
      InstructionTypes.L -> RegWriteSource.Memory,                    // Load指令从内存读取数据
      Instructions.jal -> RegWriteSource.NextInstructionAddress,      // jal指令写回下一条指令地址
      Instructions.jalr -> RegWriteSource.NextInstructionAddress      // jalr指令写回下一条指令地址
    )
  )
